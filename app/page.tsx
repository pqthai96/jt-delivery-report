...
export default function Home() {
    ...
    // Auto-scale table to fit viewport
    useEffect(() => {
        const calculateScale = () => {
            if (!tableContainerRef.current || !tableRef.current || report.length === 0) return;

            const container = tableContainerRef.current;
            const table = tableRef.current;

            // Get available height
            const availableHeight = showFilters
                ? window.innerHeight - 180
                : window.innerHeight - 100;

            // Get table's natural height
            const tableHeight = table.offsetHeight;

            // Calculate scale to fit
            if (tableHeight > availableHeight) {
                const newScale = availableHeight / tableHeight;
        ...
        // Recalculate when report or filters change
            ...
            <div className="w-full mx-auto">
                ...
                {report.length > 0 && (
                    <div className="w-full">
                        {showFilters && (
                            <div className="bg-gray-50 border border-gray-200 rounded-lg p-2 mb-2 shadow-md">
                        ...
                        >
                            <div style={{ transform: `scale(${scale})`, transformOrigin: 'top center' }}>
                                <table ref={tableRef} className="border border-black text-xs" style={{ minWidth: '1400px' }}>
                                    <thead className="tracking-wider">
                                    <tr className="bg-[#92d050] font-times text-white extra-bold">
                                        <th className="border border-black px-3 py-1 text-sm extra-bold text-white text-left py-3">
                                            Nhân viên phát kiện
                                        </th>
                                        {[
                                            "Số đơn hàng phát",
                                            ...
                                            <th
                                                key={i}
                                                className="border border-black px-3 py-1 text-sm extra-bold text-center"
                                            >
                                                {title}
                                            </th>
                                        ))}
                                    ...
                                    {filteredReport.map((row, index) => (
                                        ...
                                        >
                                            {[
                                                ...
                                                <td
                                                    key={i}
                                                    className={`border border-black px-3 py-0 text-sm text-black ${
